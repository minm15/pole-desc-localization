...
parser = argparse.ArgumentParser(description='Pole Loc')
parser.add_argument('--n_mapdetections', type=int, default=5,
                    help='number of mapdetections (default: 6)')
parser.add_argument('--mapinterval', type=float, default=0.25,
                    help='mapinterval')
parser.add_argument('--n_locdetections', type=int, default=1,
                    help='n_locdetections')
parser.add_argument('--session_start', type=int, default=0,
                    help='session_start')
parser.add_argument('--session_end', type=int, default=27,
                    help='session_end')
...
def get_locfileprefix():
    return ('localization_{:.0f}_{:.2f}_{:.2f}_{:.2f}_learning_1000_'+str(n_locdetections)).format(
    ...
    for isession, s in enumerate(pynclt.sessions):
        ...
        with progressbar.ProgressBar(max_value=len(imaps)) as bar:
            for iimap, imap in enumerate(imaps):
                ...
                localpoleparam = poles_extractor.detect_poles_learning(
                    xyz, model, device, cut_z=False)
                localpoleparam_xy = localpoleparam[:, :2]
                localpoleparam_xy = localpoleparam_xy.T
                localpoleparam_xy = np.vstack([localpoleparam_xy, np.zeros_like(
                    localpoleparam_xy[0]), np.ones_like(localpoleparam_xy[0])])  # 4*n
                localpoleparam_xy = np.matmul(
                    session.T_w_r_gt_velo[imid[imap]], localpoleparam_xy)
...
def localize(sessionname, visualize=False):
    ...
    polevar = 1.50
    session = pynclt.session(sessionname)
    locdata = np.load(os.path.join(
        session.dir, get_localmapfile()), allow_pickle=True)['maps']
    polepos_m = []
    polepos_w = []
    ...
    filter = particlefilter.particlefilter(1000,
                                           T_w_r_start, 2.5, np.radians(5.0), polemap, polevar, T_w_o=T_mc_r, d_max=1.0)
    ...
    with progressbar.ProgressBar(max_value=session.t_relodo.size) as bar:
        for i in range(istart, session.t_relodo.size):
            ...
            if imap < locdata.shape[0]:
                ...
                if t_now >= t_end:
                    imaps = range(imap, np.clip(
                        imap-n_localmaps, -1, None), -1)
                    ...
                    if iactive:
                        ...
                        T_r_now_r_mid = util.invert_ht(
                            T_w_r_now).dot(T_w_r_mid)
...
def evaluate():
    stats = []
    for sessionname in pynclt.sessions[args.session_start:args.session_end]:
        files = [file for file
                 in os.listdir(os.path.join(pynclt.resultdir, sessionname))
                 if file.startswith(get_locfileprefix())]
        files.sort()
